...
def buildClassifier(name):
    ...
    if name == "svm" or name == "linear-svm":
        ...
        classifier = RandomForestClassifier(n_estimators=64, max_features=1.0)
        predict_proba = True
        decision_function = False  
    elif name == "svm-rbf":
        classifier = SVC()
...
def evaluate(args):
    ...
    for train_dataset, test_dataset in outer_datasets: #bootstrap? (100 iterations...)       
        ...
        train = train_dataset.get_sub_dataset(args[0])
        x_train = train.X()  # data matrix
        y_train = train.Y()  # classes/labels of each sample from matrix x

        test = test_dataset.get_sub_dataset(args[0])
        x_test = test.X()
        y_test = test.Y()
        ...
        if i == k-1:
            ...
            specificity = metrics.recall_score(t,p,pos_label=0)
            kappa = metrics.cohen_kappa_score(t, p)
            f1_weighted = metrics.f1_score(t,p, average='weighted')
    ...
    return {'g':args[0], 'accs':accs, 'f1s': f1s, 'precisions': precisions,'recalls':recalls , 'specificities':specificities ,'fpr_l':fpr_l, 'tpr_l':tpr_l, 'p_tpr':processed_tpr, 'aucs':aucs, 'kappas':kappas, 'f1s_weighted':f1s_weighted}     
...
if __name__ == '__main__':
    ...
    dataset = Dataset(path_dataset+"independent_train.csv", scale=False, normalize=False, sep=',', positive="N+")

    dataset_test = Dataset(path_dataset+"independent_test.csv", scale=False, normalize=False, sep=',', positive="N+")
    #dataset_test = dataset
    
    print("\n\n\nNegative0, Positive1: "+ str(dataset.Y())+"\n\n\n\n")
    ...
    with open(path_results+'combinations/predictions_all_classifiers_n_signatures.csv', 'w')as f:            
        for name in classifiers_names:
            ...
            if independent_test_only:
                line = "Classifier, N, Kappa, F1 Weighted, Accuracy Independent Test, AUC Independent Test, F1 Independent Test, Sensitivity Independent Test, Specificity Independent Test, Precision Independent Test, Signature\n"
            else:
                line = "Classifier, N, Kappa, Kappa SD, F1 Weighted, F1 Weighted SD, Accuracy Mean, Accuracy SD, AUC Mean, AUC SD, F1, F1 SD, Sensitivity Mean, Sensitivity SD, Specificity Mean, Specificity SD, Precision Mean, Precision SD, Kappa Independent Test, F1 Weighted Independent Test, Accuracy Independent Test, AUC Independent Test, F1 Independent Test, Sensitivity Independent Test, Specificity Independent Test, Precision Independent Test, Resampling N, Resampling N SD, Resampling Min, Resampling Max, Samples, Labels, Y, Predicted, Signature\n"
            ...
            for sub_list_size in range(1, max_group_size+1):
                ...
                all_possible_groups = itertools.combinations(dataset.genes, sub_list_size)
                n_possible_groups = nCr(len(dataset.genes),sub_list_size)       
                ...
                while(hasnext):
                    ...
                    if independent_test_only:
                        ...
                        train_dataset = dataset.get_sub_dataset(g)
                        x_train = train_dataset.X()  # data matrix
                        y_train = train_dataset.Y()  # classes/labels of each sample from matrix x

                        test_dataset = dataset_test.get_sub_dataset(g)
                        ...
                        kappa_independent = metrics.cohen_kappa_score(y_test, independent_prediction)
                        f1_weighted_independent = metrics.f1_score(y_test,independent_prediction, average='weighted')
                        
                        tn, fp, fn, tp = confusion_matrix(y_test, independent_prediction).ravel()

                        specificity_independent = metrics.recall_score(y_test,independent_prediction, pos_label=0)                       
                        ...
                        if acc_independent >= min_acc_ROC and (predict_proba or decision_function):
                            ...
                            if save_ROC_graphics and auc_independent > 0.5:
                            ...
                            if independent_test_only:
                                line = name + "," +\
                                        str(len(group))+","+\
                                        str(kappa_independent)+","+\
                                        str(f1_weighted_independent)+","+\
                                        str(acc_independent)+","+\
                                        str(auc_independent)+","+\
                                        str(f1_independent)+","+\
                        ...
                        for i in range(len(result_part)):            
                            ...
                            f1_weighted = np.mean(result['f1s_weighted'])
                            f1_weighted_std = np.std(result['f1s_weighted'])
                            
                            kappa = np.mean(result['kappas'])
                            kappa_std = np.std(result['kappas'])                            
                            ...
                            if acc > min_acc_ROC and processed_tpr:
                                ...
                                if save_ROC_graphics and mean_roc_auc > min_AUC_ROC:
                                    plt.figure(figsize=(12, 11))                       
                                    plt.title(view_classifiers_names[name]+' ROC curve for '+str(group))
                                    plt.plot([0, 1], [0, 1], linestyle='--', lw=1, color='r', label='Random', alpha=.8)
                            ...
                            train_dataset = dataset.get_sub_dataset(group)            
                            x_train = train_dataset.X()  # data matrix
                            y_train = train_dataset.Y()  # classes/labels of each sample from matrix x

                            test_dataset = dataset_test.get_sub_dataset(group)
                            x_test = test_dataset.X()
                            y_test = test_dataset.Y() 
                            ...
                            if acc_independent >= min_acc_ROC and (predict_proba or decision_function):
                                ...
                                line = name + "," +\
                                            ...
                                            str(test_dataset.labels)+","+\
                                            str(test_dataset.Y())+","+\
                                            str(independent_prediction)                                                                                   
                                for g in group:
                                    line = line + "," + str(g)
                                f.write(line+"\n") 
                                f.flush()
