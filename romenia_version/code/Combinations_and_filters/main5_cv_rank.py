...
def evaluate(args):
    classifiers_class = {"svm-linear":svm.LinearSVC, "tree":tree.DecisionTreeClassifier, "nsc":NearestCentroid, "naive-bayes":GaussianNB, "glm": LogisticRegression, "sgdc":SGDClassifier,"mtElasticNet":MultiTaskElasticNet,"elasticNet":ElasticNet,"perceptron":Perceptron, "randForest":RandomForestClassifier, "svm-rbf":SVC}
...
if __name__ == '__main__':
    ...
    classifiers_names = ["nsc","svm-linear","svm-rbf", "glm", "naive-bayes", "tree"]#["svm","tree","nsc","naive-bayes","glm","sgdc","perceptron","svm-rbf"]#["svm","tree","nsc","naive-bayes","glm","sgdc","perceptron", "randForest"] #["svm","tree","nsc","naive-bayes"]
    classifiers_class = {"svm-linear":svm.LinearSVC, "tree":tree.DecisionTreeClassifier, "nsc":NearestCentroid, "naive-bayes":GaussianNB, "glm": LogisticRegression, "sgdc":SGDClassifier,"mtElasticNet":MultiTaskElasticNet,"elasticNet":ElasticNet,"perceptron":Perceptron, "randForest":RandomForestClassifier, "svm-rbf":SVC}
        ...
        for rank_name in rankes_names:
            ...
            for name in classifiers_names:            
                ...
                with open('./results/cv/cv_'+rank_name.replace('.csv','')+'_'+name+'.csv', 'w') as f:
                    ...
                    np_matrix = np.matrix(matrix_cv)
                    mean_accs = []

                    print "len matrix", len(matrix_cv)
                    for li in range(len(matrix_cv)):
                        mean_accs.append(mean(np_matrix[li,:]))
                    
                    max_acc = max(mean_accs)
                    print '---------'
                    print mean_accs
                    print max_acc
                    print mean_accs.index(max_acc)
                    print '---------'
                    n_max = mean_accs.index(max_acc)
                    n_file.write(rank_name+','+name+','+str(n_max)+','+str(max_acc)+'\n')
